...
public class WeatherDataModelBuilder
{
    ...
    public WeatherDataModelBuilder()
    {
        _faker = new Faker<WeatherDataModel>()
            .RuleFor(d => d.Latitude, f => Convert.ToSingle(f.Address.Latitude()))
            .RuleFor(d => d.Longitude, f => Convert.ToSingle(f.Address.Longitude()))
            .RuleFor(d => d.Time, f => f.Date.RecentOffset(days: 10).DateTime)
